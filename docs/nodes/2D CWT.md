...
|------|------|---------|-------------|
| wavelet | dropdown | mexican_hat | Selected 2D wavelet; two types: Gaussian and Hat |
...
- Scale is measured in pixels, matching the interactive slider which can display the scale in pixel units.
- The Fourier-space wavelet `w(k) = exp(-s²k²/2)` (Gaussian) or `w(k) = s²k²·exp(-s²k²/2)·2πs²` (Mexican hat) is sampled on the raw-FFT frequency grid with radial frequency `mval = 4·|k|/xres`.
- An interactive dialog shows the single-scale CWT as the scale slider is swept; this node freezes that behaviour into one output field by taking the element-wise maximum of the absolute single-scale responses.
